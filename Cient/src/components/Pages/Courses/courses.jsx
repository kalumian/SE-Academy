...
export default class Courses extends Component{
    ...
    componentDidMount(){
        ...
        .then(Items => {
            ...
            const SectionsTitles = this.state.dataFromDataBase.map(item => item.field)
    ...
    render(){
        ...
        function Page(){
            const infoDataArray =  dataFromDataBase.filter(item => item.field == data)
           const infoDataComponent = infoDataArray.map(item =>{
            const {title, icon, colorThem, field, description, SectionTitle, _id} = item
            return (
            <CoursCard
            key={_id}
            Icon={icon} 
        ...
        const Content =  this.state.Loading ? <LodingPage/> : this.state.TitleSections.includes(data) ?<Page/> : <NotFound/>

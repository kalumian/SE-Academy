...
export default class ShowVideo extends Component{
    ...
    componentDidMount(){
        ...
        .then(Items => {
            ...
            const CourseTitle = this.state.dataFromDataBase.map(item => [item.course_title , item.episode])
    ...
    // تاكيد صحة الرابط.
        ...
        for (let i = 0; i < data.length ; i++) {
            arr.push(this.state.TitleCoursesAndEpisode[i][0] === CourseInPath && this.state.TitleCoursesAndEpisode[i][1] === Number(EpisodeInPath))
          }
        const arr2 =  arr.indexOf(true)
        const Content =  this.state.Loding ? <LodingPage/> : arr.includes(true)   ? <Page/> : <NotFound/>
        return (
            <>
            {Content}   
        </>

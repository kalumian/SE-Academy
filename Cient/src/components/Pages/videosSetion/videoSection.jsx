...
export default class ListVideos extends Component {
  ...
  componentDidMount() {
    ...
    axios
      ...
      .then((Items) => {
        ...
        const CourseTitles = this.state.dataFromDataBase.map(
  ...
  render() {
    ...
    function Page() {
      ...
      const infoDataComponentSort = infoDataArray.sort((e, a) =>
        e.episode > a.episode ? 1 : -1
      );
    ...
    const Content = this.state.Loding ? (
      <LodingPage />
    ) : this.state.TitleCourses.includes(data) ? (
      <Page />
    ) : (
      <NotFound />
    );
